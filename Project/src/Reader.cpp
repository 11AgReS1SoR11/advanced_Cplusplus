...
std::vector<std::pair<std::string, std::string>> Reader::takeData(const std::string& path)
{
    ...
    while (std::getline(file, token, '<'))
    {
        ...
        if (name[0] == '/') { tokens.push_back(std::pair<std::string, std::string>{"", ""}); continue; } // end of name -> add empty object
...
void Reader::deleteLastSpaces(std::string& str) noexcept
{
